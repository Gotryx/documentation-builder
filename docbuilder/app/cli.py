...
def main() -> None:
    ...
    subparsers = parser.add_subparsers(
        dest="command", required=True, help="Subcomandos disponíveis"
    )

    # Comando 'create'
    create_parser = subparsers.add_parser(
        "create", help="Cria um novo projeto de documentação."
    )
    ...
    create_parser.add_argument(
        "--template", default="Corporate", help="Nome do template inicial."
    )
    create_parser.add_argument(
        "--dir", required=True, help="Diretório onde o projeto será criado."
    )

    # Comando 'validate'
    validate_parser = subparsers.add_parser(
        "validate", help="Valida a integridade do projeto e links."
    )
    validate_parser.add_argument(
        "--project", required=True, help="Caminho raiz do projeto."
    )

    # Comando 'build'
    build_parser = subparsers.add_parser(
        "build", help="Compila e exporta a documentação."
    )
    ...
    build_parser.add_argument(
        ...
        help="Formatos de saída separados por vírgula.",
    )

    # Comando 'import'
    import_parser = subparsers.add_parser(
        "import", help="Importa um arquivo externo para o diretório do projeto."
    )
    import_parser.add_argument(
        "--file", required=True, help="Caminho do arquivo externo a ser importado."
    )
    import_parser.add_argument(
        "--project", required=True, help="Caminho raiz do projeto."
    )
    ...
    ws_list_parser = subparsers.add_parser(
        "workspace-list", help="Lista todos os projetos cadastrados no Workspace."
    )
    ws_list_parser.add_argument(
        "--dir", required=True, help="Caminho raiz do workspace."
    )

    # Comando 'workspace-register'
    ws_reg_parser = subparsers.add_parser(
        "workspace-register", help="Cadastra um projeto existente no Workspace."
    )
    ws_reg_parser.add_argument("--project", required=True, help="Caminho do projeto.")
    ws_reg_parser.add_argument(
        "--dir", required=True, help="Caminho raiz do workspace."
    )

    # Comando 'sync'
    sync_parser = subparsers.add_parser(
        "sync", help="Sincroniza o projeto com o controle de versão Git."
    )
    sync_parser.add_argument("--project", required=True, help="Caminho do projeto.")
    sync_parser.add_argument(
        "--remote", default="", help="URL opcional do repositório Git remoto."
    )
    ...
    if args.command == "create":
        ...
        try:
            ...
            print(
                json.dumps(
                    {"success": True, "project": dto.__dict__},
                    ensure_ascii=False,
                    default=str,
                )
            )
        ...
        try:
            ...
            print(
                json.dumps(
                    {"success": True, "imported_document": dto.__dict__},
                    ensure_ascii=False,
                )
            )
        except Exception as e:
        ...
        try:
            ...
            success = use_case.execute(
                project_path, {"remote_url": remote_url} if remote_url else {}
            )
            print(
                json.dumps(
                    {
                        "success": success,
                        "message": "Sincronização concluída com sucesso e tag criada."
                        if success
                        else "Sincronização falhou.",
                    },
                    ensure_ascii=False,
                )
            )
