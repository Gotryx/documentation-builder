...
class GitStorage(ICloudStorage):
    ...
    def sync_project(self, project_path: Path) -> bool:
        ...
        if not self._connected:
            raise RuntimeError("Provedor Git não inicializado. Chame 'connect' primeiro.")
        ...
        try:
            ...
            if self._remote_url:
                ...
                self._run_git(project_path, ["remote", "add", "origin", self._remote_url])

            # 3. Adiciona arquivos do projeto (manifesto e subpastas de conteúdo)
            self._run_git(project_path, ["add", "manifest.yaml", "documents/", "resources/"])
            ...
            if status_res.strip():
                ...
                self._run_git(project_path, ["tag", "-a", tag_name, "-m", f"Versão {version_str}"])

                # 5. Se houver remoto, envia
                if self._remote_url:
                    # Envia ramificação atual (normalmente master ou main)
                    # Descobre ramificação atual
                    branch = self._run_git(project_path, ["rev-parse", "--abbrev-ref", "HEAD"]).strip()
                    self._run_git(project_path, ["push", "-u", "origin", branch], check=False)
                    self._run_git(project_path, ["push", "origin", "--tags"], check=False)
    ...
    def _run_git(self, cwd: Path, args: list, check: bool = True) -> str:
        ...
        if check and res.returncode != 0:
            raise RuntimeError(f"Comando Git falhou: {' '.join(cmd)}\nErro: {res.stderr.strip()}")
        return res.stdout
