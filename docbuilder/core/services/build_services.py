...
from docbuilder.core.domain.entities import Project
from docbuilder.core.domain.interfaces import ITemplateRepository, IDocumentBuilder, IExporter
from docbuilder.core.services.dtos import ProjectDTO, ValidationResultDTO, BuildResultDTO
...
class ValidateProjectUseCase:
    ...
    def execute(self, project_dto: ProjectDTO, project_dir: Path) -> ValidationResultDTO:
        errors: List[str] = []
        warnings: List[str] = []
        ...
        if project.template_name not in templates_available:
            errors.append(f"O template '{project.template_name}' configurado no projeto não existe.")

        # 3. Validar arquivo de logo (se habilitado)
        if project.logo_path:
            logo_absolute = project_dir / project.logo_path
            if not logo_absolute.exists():
                errors.append(f"O arquivo de logo '{project.logo_path}' não foi encontrado no disco.")
                ...
                for cap in part.chapters:
                    for doc in cap.documents:
                        ...
                        if not doc_path.exists():
                            ...
                            doc_errors, doc_warnings = self._validate_document_content(doc_path, project_dir)
                            errors.extend([f"[{doc.title}] {err}" for err in doc_errors])
                            warnings.extend([f"[{doc.title}] {warn}" for warn in doc_warnings])
        ...
        for dup in duplicates:
            warnings.append(f"O arquivo '{Path(dup).name}' está sendo reutilizado em múltiplos capítulos.")

        is_valid = len(errors) == 0
        return ValidationResultDTO(is_valid=is_valid, errors=errors, warnings=warnings)

    def _validate_document_content(self, file_path: Path, project_dir: Path) -> Tuple[List[str], List[str]]:
        ...
        for link in all_links:
            ...
            if link_clean.startswith(("http://", "https://", "mailto:", "ftp:", "tel:")):
                continue
            ...
            if not link_path_relative_to_file.exists() and not link_path_relative_to_project.exists():
        ...
        for img in md_images + all_sources:
            ...
            if not img_path_relative_to_file.exists() and not img_path_relative_to_project.exists():
                errors.append(f"Imagem local inexistente referenciada no texto: '{img}'")
...
class BuildProjectUseCase:
    ...
    def execute(self, project_dto: ProjectDTO, project_dir: Path, output_formats: List[str]) -> BuildResultDTO:
        self._logs = []
        ...
        try:
            ...
            compiled_doc_path = self._builder.build(project, project_dir, template_style)
            self._log(f"Estrutura montada com sucesso em arquivo temporário: {compiled_doc_path.name}")
        except Exception as e:
            self._log(f"ERRO CRÍTICO NO BUILDER: {e}")
            return BuildResultDTO(success=False, message=f"Erro de compilação: {e}", logs=self._logs)
        ...
        for fmt in output_formats:
            ...
            if not exporter:
                self._log(f"Aviso: Nenhum exportador disponível para o formato '{fmt}'. Ignorado.")
                continue

            # Nome do arquivo final gerado
            clean_project_name = "".join(c for c in project.name if c.isalnum() or c in (" ", "_", "-")).rstrip()
        ...
        message = "Build finalizado com sucesso!" if success else "O build finalizou mas nenhuma exportação pôde ser gerada."
        return BuildResultDTO(success=success, message=message, output_files=output_files, logs=self._logs)
