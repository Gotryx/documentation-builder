...
class HtmlExporter(IExporter):
    ...
    def export(
        ...
        html_lines.append(
            f"    <title>{escape(template.header_text or 'Documentação GoTryx')}</title>"
        )
        ...
        for block in blocks:
            if isinstance(block, SectionBreakBlock):
                html_lines.append(
                    f'        <section class="document-section" data-title="{escape(block.title)}">'
                )

            elif isinstance(block, HeadingBlock):
                level = min(max(block.level, 1), 6)
                html_lines.append(
                    f"            <h{level}>{escape(block.text)}</h{level}>"
                )
