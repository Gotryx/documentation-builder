...
class DocxExporter(IExporter):
        ...
        for idx, block in enumerate(blocks):
            if isinstance(block, SectionBreakBlock):
                ...
                for i, col_header in enumerate(table_block_headers):
                    ...
                    self._style_table_cell(hdr_cells[i], template.primary_color, is_header=True, font_family=template.font_family, font_size=template.font_size)
                ...
                for r_idx, row_cells_data in enumerate(block.rows):
                    ...
                    for c_idx, cell_data in enumerate(row_cells_data):
                        ...
                        self._style_table_cell(row_cells[c_idx], bg_color, is_header=False, font_family=template.font_family, font_size=template.font_size)
                ...
                try:
                    ...
                    err_run = p.add_run(f"[Erro ao carregar imagem: {block.image_path} - {e}]")
                    err_run.bold = True
    ...
    def _apply_header_footer(self, section, template: TemplateStyle, section_title: str) -> None:
        ...
        if len(text_footer_parts) > 1:
            ...
            suffix_text = text_footer_parts[1].replace("#TOTAL#", "")  # Total dinâmico requer NUMPAGES
    ...
    def _style_table_cell(self, cell, bg_hex_color: str, is_header: bool, font_family: str, font_size: int) -> None:
        ...
        shading = parse_xml(f'<w:shd {nsdecls("w")} w:fill="{bg_hex_color.replace("#", "")}"/>')
        ...
        for margin in ['top', 'bottom', 'left', 'right']:
            ...
            node.set(qn('w:w'), '120' if margin in ['top', 'bottom'] else '180')  # tamanho do padding
        ...
        for p in cell.paragraphs:
            ...
            for run in p.runs:
                ...
                if is_header:
                    run.bold = True
                    run.font.color.rgb = RGBColor(255, 255, 255)  # Letras brancas no cabeçalho
                else:
